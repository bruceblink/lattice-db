...
#[path = "stage_zero_model.rs"]
mod stage_zero_model;
#[path = "stage_zero_validation.rs"]
mod stage_zero_validation;

use stage_zero_model::{
    Durability, FaultCase, FaultMatrix, FrameTypes, FrozenSpec, TransitionRule,
};
use stage_zero_validation::{
    validate_filesystem, validate_frame_layout, validate_identifiers, validate_sequences,
};
...
/// Deterministic result of checking the embedded stage-0 specification.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct StageZeroSummary {
    /// Version string accepted from the frozen specification.
    pub spec_version: String,
    /// Count of validated fault cases.
    pub fault_cases: usize,
    /// Count of validated transition rules.
    pub transition_rules: usize,
    /// Stable fingerprint of exact specification and matrix bytes.
    pub spec_hash: String,
}

/// Separates malformed TOML from a readable but unsafe specification.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Parses embedded documents, validates cross-field invariants, and hashes exact bytes.
...
/// Decodes one TOML document and preserves its role in parse errors.
...
/// Checks scalar compatibility, capacities, structural rules, and transitions.
fn validate_spec(spec: &FrozenSpec) -> Result<(), StageZeroError> {
    ...
    require(
        spec.frame_header_bytes == 32,
        "frame_header_bytes must be 32",
    )?;
    validate_identifiers(&spec.identifiers)?;
    validate_sequences(&spec.sequences)?;
    validate_filesystem(&spec.filesystem)?;
    validate_frame_layout(&spec.frame_layout)?;
...
/// Checks the fixed numeric frame type table and rejects duplicate values.
...
/// Requires a persistence explanation for each visible synchronization state.
...
/// Checks the exact five-edge monotonic status graph and its explanations.
...
/// Checks fault identities, categories, and references to frozen vocabulary.
...
/// Checks non-empty fault descriptions and vocabulary references.
fn validate_fault_case(
    ...
    require(
        !case.cursor_effect.contains(" or ") && !case.cursor_effect.contains(" may "),
        &format!("{} has ambiguous cursor effect", case.id),
    )?;
    require(
...
/// Returns a stable FNV-1a fingerprint over spec, NUL, and matrix bytes.
